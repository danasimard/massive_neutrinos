...
\end{equation}
and
\begin{equation}\label{eqn:euler6} 
...
CDM and neutrino 
density power spectra from the Euler equation \eqref{eqn:euler5} and
the continuity equation \eqref{eqn:continuity4}, derived in Section
...
\eqref{eqn:euler5} and differentiating the continuity equation
\eqref{eqn:continuity4} with respect to time, $t$. Combining these equations, we obtain
...
%\end{equation}
%\end{aside}
\begin{equation}\label{eqn:densitycontrast} 
  \frac{1}{a^2(t)} \frac{d}{dt}\bigg( a^2(t) \frac{ d
  }{dt}\delta_i(\mathbf{k},t) \bigg) - \frac{\nabla^2 P_i}{a^2(t)
  \widebar{\rho_i}(t)} = \frac{1}{a^2(t)}\frac{3}{2} H^2(t) a^2(t) \delta_c(\mathbf{k},t) \;.
\end{equation}
...
for the significant thermal motion of neutrinos.  We will
...
\begin{equation}\label{eqn:densitypowercdm}
  P_c(\tau) = < \delta_c(\tau) \delta_c^*(\tau)> = \int_{-\infty}^\tau
 d\tau' (\tau-\tau') \frac{3}{2} H^2(\tau') \int_{-\infty}^\tau d\tau'' (\tau-\tau'') \frac{3}{2}
...
\begin{equation}\label{eqn:densitypowernu} 
  P_\nu(\tau) = < \delta_\nu(\tau) \delta_\nu^*(\tau) > =
...
\begin{equation} 
\begin{array}{rcl}
  H_M(\tau) &=& \frac{1}{a} \frac{ d}{d \tau} \frac{da}{dt} \\[1em]
...
\begin{equation}\label{eqn:nuearly}
\begin{array}{rcl}
...
%      \frac{1}{\tau'^4}  \\[1em]
  & = &\frac{24 k^2 \widebar{c_s}^2 }{ H_0^2 \Omega_M} A(k)
        ...
        \cos(\tau-\tau_0)+2 \tau_0^3
        \cos(\tau) \text{Ci}(-\tau_0)+\\[1em]
&&
  \pi
        \tau_0^3 \sin(\tau)+2
        \left(-2+\tau_0^2\right)
...
\frac{1}{12} \bigg( \frac{2}{\tau^2} + 2 \cos( \tau ) \text{CI}( -
...
\begin{equation}
  \label{eqn:densbymode}
\begin{array}{rcl}
  P_i(k,\tau) &=& \bigg| I_{i,M}(k,\tau, \tau_0) + \int_{\tau_0}^{\tau} d \tau'
  G_i(\tau-\tau') \frac{3}{2} H^2(\tau') w^0(k,\tau') \bigg |^2 +\\[1em]
&&  \sum_{m=1}^{m_{max}} \bigg| \int_{\tau_0}^\tau d\tau' G_i(\tau -
  \tau' ) \frac{3}{2} H^2 (\tau') w^m(k,\tau') \bigg |^2 \;,
\end{array}
\end{equation}
where we use the analytic integrals $I_{i,M}$ from Equations
\eqref{eqn:cdmearly} and \eqref{eqn:nuearly}.
The remaining integrals were calculated numerically using a trapezoidal
...
\begin{figure*}[h!]
  ...
  \begin{subfigure}[t]{0.495\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Figures/density_nu0p2_0.eps}
    \caption{$\sum m_\nu = 0.2$ eV}
    \label{fig:delta_nu0p2_rec}
  \end{subfigure}
  \begin{subfigure}[t]{0.495\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Figures/density_nu_0p1.eps}
    \caption{$\sum m_\nu = 0.1$ eV }
    \label{fig:delta_nu0p05_rec}
  \end{subfigure}
...
\begin{array}{rcl}
  ...
  &=& \bigg( \frac{k \widebar{c_s} }{a(\tau)} \bigg)^2 \int_{-\infty}^\tau d
      ...
      (\tau'') <\delta_c(\mathbf{k},\tau')\delta^*_c(\mathbf{k},\tau'')>\\[1em]
  &=& \bigg(\frac{k \widebar{c_s}}{a(\tau)} \bigg)^2 \sum_m \bigg|
      \int_{-\infty}^\tau d\tau' \cos( \tau-\tau') \frac{3}{2}
...
we obtain:
\begin{equation}\label{eqn:veldivgnu3} 
 \begin{array}{rcl}
...
\begin{equation} \label{eqn:veldivgnu4} 
\begin{array}{rcl}
...
\begin{equation}\label{eqn:veldivgnu5} 
\begin{array}{rcl}
  P_{{\Theta_\nu,M}}(k,\tau) &=& \bigg( \frac{k
    \widebar{c_s}}{a(\tau)}\bigg)^2 \bigg| \frac{ 24 k^2 \widebar{c_s}^2
    A(k)}{H_0^2 \Omega_M} \frac{1}{12} \bigg( \pi \tau_0^3 \cos(\tau) + 2 ( \tau_0^2 - 2) \cos(
        \tau-\tau_0) - \\[1em]
&& 2 \tau_0 ( \sin( \tau-\tau_0) + \tau_0^2 (
        \text{CI}(-\tau_0) \sin(\tau) - \cos( \tau) \text{SI}( \tau_0)
        ) ) \bigg) \bigg|^2\;.
